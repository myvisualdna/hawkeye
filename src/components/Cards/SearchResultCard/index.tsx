...
const SearchResultCard = ({data}: Props) => {
  const { galleryImgs, id, href, like, saleOff, address, price, features } = data;
  console.log(data)
    ...
    return (
        ...
        <div>
          ...
          <div>
            <a
              href="#"
              className="text-gray-500 hover:gray-800 lg:text-sm transition duration-100 mb-1"
            >
              {/* {2841 Main St., Miami - FL, 29014} */}
              {address.title} 
              <div
              className="text-gray-500 hover:gray-800 lg:text-sm transition duration-100"
            >
            ...
            <div className="flex items-end gap-2">
              <span className="text-gray-800 lg:text-base font-medium">{priceFormatter.format(price)}</span>
            </div>
          </div>
        </div>
      </Link>
